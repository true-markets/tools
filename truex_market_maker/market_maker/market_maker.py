...
class OrderManager:
    ...
    def _process_order_amendment(
        ...
        Args:
            reactive_trigger: True if this amendment is due to reactive price movement detection
        """
        try:
            ...
            # Use more sensitive threshold when triggered by reactive price movements
            if reactive_trigger and self.order_adjustment_engine:
                # For reactive adjustments, use the same threshold that triggered the detection
                min_change_threshold = self.order_adjustment_engine.price_move_threshold
                logger.debug(
                    f"🚨 Using reactive threshold {min_change_threshold:.5f} for {symbol}"
                )
            else:
                # Use larger threshold for regular amendments to prevent oscillation
                min_change_threshold = max(
                    settings.RELIST_INTERVAL, 0.005
                )  # At least 0.5%
            ...
            if qty_change_needed or price_change_needed:
                ...
                threshold_type = "reactive" if reactive_trigger else "regular"
                logger.debug(
                    f"📝 Amending order {order['id']}: {', '.join(amendment_reason)} (Δ{price_change_pct:.3f}%, {threshold_type} threshold: {min_change_threshold:.5f})"
                )
            ...
            for symbol in settings.SYMBOLS:
                ...
                if last_pricing:
                    ...
                    if price_move_detected:
                        ...
                        self.place_orders([symbol], reactive_trigger=True)
    ...
    def place_orders(self, symbols=settings.SYMBOLS, reactive_trigger=False):
        """Create order items for use in convergence.

        Args:
            symbols: List of symbols to place orders for
            reactive_trigger: True if called due to reactive price movement detection
        """
        ...
        for symbol in symbols:
            ...
            self.converge_orders(symbol, buy_orders, sell_orders, reactive_trigger)
    ...
    def converge_orders(self, symbol, buy_orders, sell_orders, reactive_trigger=False):
